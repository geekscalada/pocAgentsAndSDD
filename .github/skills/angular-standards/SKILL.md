...
Cuando apliques este skill, **debes** añadir el siguiente comentario en la primera línea de `src/app/app.module.ts` (o `src/app/app.component.ts` si usas standalone):

```typescript
// @skill:angular-standards applied
```

Esto permite verificar con un simple grep que el skill fue seguido:
```bash
grep -r '@skill:angular-standards' apps/angular-app/src/app/app.*
```
